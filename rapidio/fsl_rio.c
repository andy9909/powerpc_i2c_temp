...
{
	if(type==1) //DDR memory
		rio_target_type=0x80f55000;
	else
		rio_target_type=0x80044000;
}
static void set_target_value(u32 value)
{
	rio_target_type = value;
}

static u32 rioMapOutb(struct rio_mport *mport, u32 localAddr, u32 srioAddr, u32 size,
                  u32 dstID, u32 flags, u32 uiOub)
{
	struct rio_priv *priv = mport->priv;
    u32  *uiBaseAddr,tmp_addr;
    u32  uiLawSize = (ilog2(size) - 1);
    u32  uiVal = 0;

	printk("jg uiLawSize %x,%p\n",uiLawSize,&priv->maint_atmu_regs->rowtar);
	tmp_addr = (u32)&priv->maint_atmu_regs->rowtar;
	printk("jg tmp_addr %x\n",tmp_addr);
	tmp_addr = tmp_addr + uiOub * 0x20;
    uiBaseAddr = (u32*)tmp_addr;  //target address
    printk("jg uiBaseAddr %p\n",uiBaseAddr);
    uiVal = ((dstID & 0xff) << 22) | (srioAddr >> 12);
    out_be32(uiBaseAddr,uiVal);
	
    tmp_addr = (u32)&priv->maint_atmu_regs->rowbar;//OCN address
	tmp_addr = tmp_addr + uiOub * 0x20;
    uiBaseAddr = (u32*)tmp_addr;
	printk("jg uiBaseAddr %p\n",uiBaseAddr);
    out_be32(uiBaseAddr,(localAddr >> 12));
	
    tmp_addr = (u32)&priv->maint_atmu_regs->rowar;//Attribute
	tmp_addr = tmp_addr + uiOub * 0x20;
    uiBaseAddr = (u32*)tmp_addr;
	printk("jg uiBaseAddr %p\n",uiBaseAddr);
	out_be32(uiBaseAddr,(0x80044000 | uiLawSize));
    
    return 1;
}
static u32 rioMapInb(struct rio_mport *mport, u32 localAddr, u32 srioAddr, u32 size, u32 flags, u32 uiInb)
{
	struct rio_priv *priv = mport->priv;
    u32  uiLawSize = (ilog2(size) - 1);
    u32  *uiBaseAddr,tmp_addr;
	printk("jg uiLawSize %x\n",uiLawSize);
	tmp_addr = (u32)&priv->atmu_regs->rowtar;
	printk("jg tmp_addr %x\n",tmp_addr);
    uiBaseAddr =(u32*) (tmp_addr + 0x160 + uiInb * 0x20); //target address
    printk("jg uiBaseAddr %p\n",uiBaseAddr);
    out_be32(uiBaseAddr,(localAddr >> 12));

    uiBaseAddr = (u32*) (tmp_addr  + 0x168 + uiInb * 0x20); //OCN address
    printk("jg uiBaseAddr %p\n",uiBaseAddr);
    out_be32(uiBaseAddr,(srioAddr >> 12));

    uiBaseAddr = (u32*) (tmp_addr  + 0x170 + uiInb * 0x20); //Attribute
    printk("jg uiBaseAddr %p\n",uiBaseAddr);
	out_be32(uiBaseAddr,(rio_target_type | uiLawSize));
                  ...
                  u32 uiInb)
{
	...
	rioMapInb(mem_mport,localAddr,srioAddr,size,0,uiInb);
}
...
{
	return;
/* BEGIN: Added by niefei, 2013/11/29   问题单号:niefei delect */
//	rio_init_mports();
...
void set_inb_target(unsigned char * str)
{
	u32 data;
	u32 len;
	...
	printk("jg data %x\n", data);
	if(data>1)
		set_target_value(data);
	else
		set_target_type(data);
}

void test_doorbell(unsigned char * str)
{
	u32 destid,data;
	unsigned char *p=str;
	u32 len;
...
{
	u32 baddr,taddr,size,destid,window;
	unsigned char *p=str;
	u32 len;
	printk("jg test_riooutb %s\n",str);
    ...
    window = str2hex(str);
	set_riooutb((u32)baddr,taddr,size,destid,window); //(0xc0000000+0x400000)
}

void test_rioinb(unsigned char * str) //localaddr rioaddr size window(0~3)
{
	u32 baddr,taddr,size,window;
	unsigned char *p=str;
	u32 len;
	printk("jg test_rioinb %s\n",str);
    ...
    window = str2hex(str);
	set_rioinb((u32)baddr,taddr,size,window); //0x1000000 0x40000000
}
...
void test_memory_map(void)
{
	...
	test_m = (void*)0xe1000000;
	//memset(test_m,0,TEST_SIZE);
	printk("jg test_memory_map\n");
...

{
	...
	printk("niefei test_sendMsg%s\n",str);

	p=strsep((char**)&str,(char*)" ");
	usDestId=(unsigned short)str2hex(p);
...
/*****************************************************************************
 ...
 时间:2013-11-26
*****************************************************************************/
 void testInBoundGetMsg(unsigned char * str)

{
	...
	if(str=='\0')
	{
		printk(" recv msg argument error!\n");
		return;
	}
