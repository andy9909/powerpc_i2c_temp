					...
					struct nfsd_diropres   *resp)
{
	...
	printk("===>[%s]\n", __func__);
	printk("housir:valid[0x%x]mode[0x%x]uid[0x%x]gid[0x%x]size[0x%x]\n", (argp->attrs).ia_valid, (argp->attrs).ia_mode,  (argp->attrs).ia_uid,  (argp->attrs).ia_gid,  (argp->attrs).ia_size);
